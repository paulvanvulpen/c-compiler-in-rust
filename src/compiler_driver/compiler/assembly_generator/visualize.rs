...
impl visualize::Visualizer for assembly_generator::FunctionDefinition {
	fn visualize(&self, depth : u8) -> String {
		let assembly_generator::FunctionDefinition::Function{identifier, instructions} = self;
...
impl visualize::Visualizer for assembly_generator::Instruction {
	fn visualize(&self, depth : u8) -> String {
		match self {
			assembly_generator::Instruction::Mov(op1, op2) => format!("Mov({}, {})", op1.visualize(depth + 1), op2.visualize(depth + 1)),
			assembly_generator::Instruction::Unary(unary_operator, dst) => format!("Unary({}, {})", unary_operator.visualize(depth + 1), dst.visualize(depth + 1)),
			assembly_generator::Instruction::AllocateStack(int) => format!("AllocateStack({})", int),
